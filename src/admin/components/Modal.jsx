    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          />
          <div className="fixed inset-0 z-[901] flex items-center justify-center p-4 sm:p-6 pointer-events-none">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className={`w-full ${sizeClasses[size]} bg-[#111] border border-white/10 rounded-[24px] shadow-2xl pointer-events-auto flex flex-col max-h-[90vh] overflow-hidden`}
            >
